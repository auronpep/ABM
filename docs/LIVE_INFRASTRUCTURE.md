...
- **Enrollment email silently skips** if `RESEND_API_KEY` / `BARMATRIX_SUPPORT_EMAIL` / `BARMATRIX_REPLY_TO_EMAIL` aren't set in prod secrets (status "skipped", reason "missing_config"). For internal testing, send test mail to `codex@barmatrix.app`; it is the BarMatrix test mailbox and domain catchall. Mailbox settings live in `C:\Users\JesusLovesMe\.env`; the password is stored under `CODEX_EMAIL_PASSWORD` and must not be copied into repo docs or commits.
...
**Env keys** (names only): API requires 15 at boot incl. `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRODUCT_BARMATRIX_FLAGSHIP`, `STRIPE_PRICE_PAY_IN_FULL`, `STRIPE_PRICE_FLAGSHIP_ANCHOR`, `STRIPE_PRICE_PAY_IN_TWO`, `STRIPE_PRICE_PAY_IN_TWO_SECOND`, `CLERK_PUBLISHABLE_KEY`, `CLERK_SECRET_KEY`, DB vars, `FRONTEND_URL`/`SUCCESS_URL`/`CANCEL_URL`, `ALLOWED_ORIGINS`. Optional: Resend trio, Sentry, `INTERNAL_JOB_SECRET`. Local test mailbox credentials/settings are outside the repo in `C:\Users\JesusLovesMe\.env`; use `CODEX_EMAIL_PASSWORD` for `codex@barmatrix.app`. Full list: `C:\barmatrix-canon\20-data\env-keys.md`.

**Email testing mailbox**: `codex@barmatrix.app` is the default QA recipient for email flows and is also the domain catchall for BarMatrix test mail. Use it for checkout/enrollment, welcome, abandoner, prayer-chain, weekly-repair, support/contact, and provider deliverability tests unless a test case specifically requires another address. Never print the password value; load it from `C:\Users\JesusLovesMe\.env` as `CODEX_EMAIL_PASSWORD`.
